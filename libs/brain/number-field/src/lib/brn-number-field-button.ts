...
export abstract class BrnNumberFieldButton {
	...
	protected _onPointerDown(event: PointerEvent): void {
		...
		this._numberField.focus();

		this._step();

		if (!this._numberField.holdRepeat()) return;
		...
		this._delayTimeout = setTimeout(() => {
			this._repeatInterval = setInterval(() => {
				...
				this._step();
			}, this._numberField.holdRepeatInterval());
		}, this._numberField.holdRepeatDelay());
	}
