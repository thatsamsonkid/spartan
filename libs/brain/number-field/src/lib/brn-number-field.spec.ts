...
import { BrnNumberFieldImports } from '../index';

@Component({
	imports: [BrnNumberFieldImports],
	...
	template: `
		<div
			...
			aria-label="Quantity"
		>
			<button brnNumberFieldDecrement>-</button>
			<button brnNumberFieldIncrement>+</button>
		</div>
	`,
})
class SpinbuttonNumberField {
...
@Component({
	...
	changeDetection: ChangeDetectionStrategy.OnPush,
	template: `
		<div brnNumberField [value]="value" [min]="min" [max]="max" aria-label="Quantity">
			<button brnNumberFieldDecrement>-</button>
			<input brnNumberFieldInput />
			<button brnNumberFieldIncrement>+</button>
		</div>
	`,
})
...
@Component({
	imports: [BrnNumberFieldImports, ReactiveFormsModule],
	changeDetection: ChangeDetectionStrategy.OnPush,
	template: `
		<div brnNumberField [min]="0" [max]="10" [formControl]="control" aria-label="Quantity">
			<button brnNumberFieldDecrement>-</button>
			<input brnNumberFieldInput />
			<button brnNumberFieldIncrement>+</button>
		</div>
...
class ReactiveFormNumberField {
	public readonly control = new FormControl<number | null>(5);
}

describe('BrnNumberField', () => {
	describe('spinbutton mode (no input)', () => {
		it('should place the spinbutton role and value aria attributes on the root', async () => {
			const { container } = await render(SpinbuttonNumberField);
			const root = container.querySelector('[brnNumberField]') as HTMLElement;
		...
		it('should apply a 10x multiplier with shift and page keys', async () => {
			const { container } = await render(SpinbuttonNumberField, { componentProperties: { max: 100 } });
			const root = container.querySelector('[brnNumberField]') as HTMLElement;
		...
		it('should clamp keyboard changes to the bounds', async () => {
			const { container } = await render(SpinbuttonNumberField, { componentProperties: { value: 10 } });
			const root = container.querySelector('[brnNumberField]') as HTMLElement;
		...
		it('should not react to keyboard when disabled', async () => {
			const { container } = await render(SpinbuttonNumberField, { componentProperties: { disabled: true } });
			const root = container.querySelector('[brnNumberField]') as HTMLElement;
		...
		it('should normalize an out of bounds initial value', async () => {
			const { container } = await render(SpinbuttonNumberField, { componentProperties: { value: 50 } });
			const root = container.querySelector('[brnNumberField]') as HTMLElement;
	...
	describe('increment and decrement buttons', () => {
		it('should step the value on pointer down', async () => {
			const { container } = await render(SpinbuttonNumberField);
			const root = container.querySelector('[brnNumberField]') as HTMLElement;
			const increment = container.querySelector('[brnNumberFieldIncrement]') as HTMLButtonElement;
			const decrement = container.querySelector('[brnNumberFieldDecrement]') as HTMLButtonElement;
		...
		it('should disable both buttons when the number-field is disabled', async () => {
			const { container } = await render(SpinbuttonNumberField, { componentProperties: { disabled: true } });
			const increment = container.querySelector('[brnNumberFieldIncrement]') as HTMLButtonElement;
			const decrement = container.querySelector('[brnNumberFieldDecrement]') as HTMLButtonElement;
		...
		it('should repeat the step while holding the button', async () => {
			...
			try {
				const { fixture, container } = await render(SpinbuttonNumberField, { componentProperties: { max: 100 } });
				const root = container.querySelector('[brnNumberField]') as HTMLElement;
				const increment = container.querySelector('[brnNumberFieldIncrement]') as HTMLButtonElement;
		...
		it('should stop repeating at the bound', async () => {
			...
			try {
				const { fixture, container } = await render(SpinbuttonNumberField, { componentProperties: { value: 8 } });
				const root = container.querySelector('[brnNumberField]') as HTMLElement;
				const increment = container.querySelector('[brnNumberFieldIncrement]') as HTMLButtonElement;
	...
	describe('input mode', () => {
		it('should move the spinbutton role onto the input and make the root a group', async () => {
			const { container } = await render(InputNumberField);
			const root = container.querySelector('[brnNumberField]') as HTMLElement;
	...
	describe('reactive forms', () => {
		it('should reflect the control value', async () => {
			const { container } = await render(ReactiveFormNumberField);
			const input = container.querySelector('input') as HTMLInputElement;
		...
		it('should update the control when stepping', async () => {
			const { fixture, container } = await render(ReactiveFormNumberField);
			const increment = container.querySelector('[brnNumberFieldIncrement]') as HTMLButtonElement;
		...
		it('should disable the number-field when the control is disabled', async () => {
			const { fixture, container } = await render(ReactiveFormNumberField);
			const input = container.querySelector('input') as HTMLInputElement;
			const increment = container.querySelector('[brnNumberFieldIncrement]') as HTMLButtonElement;
