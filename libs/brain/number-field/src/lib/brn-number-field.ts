...
import type { BrnNumberFieldInput } from './brn-number-field-input';
import { provideBrnNumberField } from './brn-number-field.token';

export const BRN_NUMBER_FIELD_VALUE_ACCESSOR = {
	provide: NG_VALUE_ACCESSOR,
	useExisting: forwardRef(() => BrnNumberField),
	multi: true,
};

export type BrnNumberFieldLabelFn = (value: number) => string;
...
@Directive({
	selector: '[brnNumberField]',
	exportAs: 'brnNumberField',
	providers: [BRN_NUMBER_FIELD_VALUE_ACCESSOR, provideBrnNumberField(BrnNumberField)],
	hostDirectives: [BrnFieldControl],
	host: {
		'[attr.id]': 'id()',
		'data-slot': 'number-field',
		'[attr.role]': 'numberFieldInput() ? "group" : "spinbutton"',
		'[attr.tabindex]': 'numberFieldInput() || mutableDisabled() ? null : 0',
		'[attr.aria-label]': 'numberFieldInput() ? null : ariaLabel()',
		'[attr.aria-labelledby]': 'numberFieldInput() ? null : ariaLabelledby()',
		'[attr.aria-valuemin]': 'numberFieldInput() ? null : min()',
		'[attr.aria-valuemax]': 'numberFieldInput() ? null : max()',
		'[attr.aria-valuenow]': 'numberFieldInput() ? null : value()',
		'[attr.aria-valuetext]': 'numberFieldInput() ? null : valueLabel()',
...
export class BrnNumberField implements ControlValueAccessor, OnInit {
	...
	/** Unique identifier for the number-field element. Auto-generated if not provided. */
	public readonly id = input<string>(`brn-number-field-${++nextId}`);

	/** Accessibility label for the number-field. Forwarded to the input when one is present. */
	public readonly ariaLabel = input<string | null>(null, { alias: 'aria-label' });

	/** ID of the element that labels this number-field for accessibility. Forwarded to the input when one is present. */
	public readonly ariaLabelledby = input<string | null>(null, { alias: 'aria-labelledby' });

	/** The current number-field value. `null` represents an empty state. */
	...
	public readonly getValueLabel = input<BrnNumberFieldLabelFn | null>(null);
	...
	/** @internal Reference to the registered number-field input, if any. */
	public readonly numberFieldInput = signal<BrnNumberFieldInput | null>(null);
	...
	/** @internal Marks the number-field as touched. */
	markAsTouched(): void {
		this._onTouched?.();
	}
	...
	/** @internal */
	registerInput(input: BrnNumberFieldInput): void {
		this.numberFieldInput.set(input);
	}

	/** @internal */
	unregisterInput(input: BrnNumberFieldInput): void {
		if (this.numberFieldInput() === input) {
			this.numberFieldInput.set(null);
		}
	}

	/** @internal Focuses the spinbutton element (the input when present, otherwise the root). */
	focus(): void {
		const target = this.numberFieldInput()?.elementRef.nativeElement ?? this.elementRef.nativeElement;
		target.focus();
	}
	...
	protected _onKeydown(event: KeyboardEvent): void {
		...
		if (this.numberFieldInput()) return;
